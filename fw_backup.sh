...
mkdir -p "$WORKDIR/$HOST/$DATE/nftables"
cp -a /etc/nftables/*.nft \
"$WORKDIR/$HOST/$DATE/nftables/" 2>/dev/null

#
# FRR
#

mkdir -p "$WORKDIR/$HOST/$DATE/frr"
cp -a /etc/frr/*.conf \
"$WORKDIR/$HOST/$DATE/frr/" 2>/dev/null
